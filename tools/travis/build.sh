...
if [[ ${TRAVIS_OS_NAME} == "linux" ]]; then
    scons $J CC=$CC CXX=$CXX compiler=$CC -k
    # scons $J CC=$CC CXX=$CXX compiler=$CC test
elif [[ ${TRAVIS_OS_NAME} == "osx" ]]; then
    echo "ls /usr/local/Cellar"
    ls /usr/local/Cellar

    # /usr/local/Cellar/numpy/1.8.2/lib/python2.7/site-packages/numpy/core/include
    export INCDIR_PYTHON_NUMPY=`echo /usr/local/Cellar/numpy/*/lib/python2.7/site-packages/numpy/core/include`
    echo "INCDIR_PYTHON_NUMPY=$INCDIR_PYTHON_NUMPY"

    scons $J CC=$CC CXX=$CXX compiler=$CC incdir_python=/System/Library/Frameworks/Python.framework/Versions/2.7/include/python2.7 incdir_python_numpy=$INCDIR_PYTHON_NUMPY -k
