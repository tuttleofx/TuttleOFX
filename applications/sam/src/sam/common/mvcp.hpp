...
int sammvcp(int argc, char** argv) {
    ...
    if (vm.count(kHelpOptionLongName) || isPathSizeTooSmall) {
        ...
        TUTTLE_LOG_INFO( color->_blue << "TuttleOFX project [http://sites.google.com/site/tuttleofx]" << color->_std << std::endl);
#ifndef SAM_MOVEFILES
        TUTTLE_LOG_INFO( color->_blue <<"NAME" << color->_std);
        TUTTLE_LOG_INFO( color->_green << "\tsam-cp - copy sequence(s) in a directory" << color->_std << std::endl);
        TUTTLE_LOG_INFO( color->_blue << "SYNOPSIS" << color->_std);
        TUTTLE_LOG_INFO( color->_green << "\tsam-cp [options] sequence[s] [outputDirectory][outputSequence]" << color->_std << std::endl);
#else
        TUTTLE_LOG_INFO( color->_blue << "NAME" << color->_std );
        TUTTLE_LOG_INFO( color->_green << "\tsam-mv - move sequence(s) in a directory" << color->_std << std::endl );
        TUTTLE_LOG_INFO( color->_blue << "SYNOPSIS" << color->_std );
        TUTTLE_LOG_INFO( color->_green << "\tsam-mv [options] sequence[s] [outputDirectory][outputSequence]" << color->_std << std::endl );
#endif
        TUTTLE_LOG_INFO( color->_blue << "DESCRIPTION" << color->_std << std::endl);
#ifndef SAM_MOVEFILES
        TUTTLE_LOG_INFO( "Copy sequence of image files, and could remove trees (folder, files and sequences)." << std::endl);
#else
        TUTTLE_LOG_INFO( "Move sequence of image files, and could remove trees (folder, files and sequences)." << std::endl );
#endif
