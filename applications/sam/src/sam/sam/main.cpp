...
bfs::path retrieveToolFullPath(const std::string& toolName,
	...
	/// @todo exception ?
	TUTTLE_CERR(
			_color._red << "Sam command \"" << toolName << "\" not found." << _color._std);
...
std::vector<bfs::path> retrieveAllSamCommands(
...
int main(int argc, char** argv) {
	...
	try {
		...
		infoOptions.add_options()(kHelpOptionString.c_str(),
				kHelpOptionMessage.c_str())(kVersionOptionString.c_str(),
				kVersionOptionMessage.c_str()) // which version?
		(kCommandsOptionString.c_str(), kCommandsOptionMessage.c_str())(
				kColorOptionString.c_str(), kColorOptionMessage.c_str());

		// describe hidden options
		bpo::options_description hidden;
		hidden.add_options()(kCommandsListOptionString.c_str(),
				kCommandsListOptionMessage.c_str())(
				kBinariesListOptionString.c_str(),
				kBinariesListOptionMessage.c_str())(
				kEnableColorOptionString.c_str(), bpo::value<std::string>(),
		...
		if (const char* env_sam_options = std::getenv("SAM_OPTIONS")) {
			const std::vector<std::string> envOptions = bpo::split_unix(
					env_sam_options, " ");
			bpo::store(
		...
		const std::vector<bfs::path> searchPaths = retrieveSearchPaths(
				samDirectory);
		...
		if (sam_vm.count(kHelpOptionLongName.c_str()) || (argc == 1)) {
			...
			TUTTLE_COUT(
					_color._green <<"\tsam - A set of command line tools." << _color._std << std::endl);
			TUTTLE_COUT( _color._blue << "SYNOPSIS" << _color._std);
			TUTTLE_COUT(
					_color._green << "\tsam COMMAND [options]..." << _color._std << std::endl);
			TUTTLE_COUT( _color._blue << "COMMANDS" << _color._std);
			const std::vector<bfs::path> cmds = retrieveAllSamCommands(
					searchPaths);
			...
			TUTTLE_COUT(
					"Sam is the TuttleOFX command line tool to manage image processing." << std::endl);
		...
		if (sam_vm.count(kVersionOptionLongName.c_str())) {
			TUTTLE_COUT(
					"TuttleOFX Host - version " << TUTTLE_HOST_VERSION_STR);
			exit(0);
		}
...
//		TUTTLE_TCOUT_VAR( fullcmd );
		...
		TUTTLE_CERR(
				_color._red << "Error in command line: " << e.what() << _color._std);
		exit(-2);
	} catch (...) {
		TUTTLE_CERR(
				_color._red << "Error in command line: " << boost::current_exception_diagnostic_information() << _color._std);
		exit(-2);
	}
	return 0;
}
