...
EImageStatus diffImageStatus(Graph::Node& read1, Graph::Node& read2,
		Graph::Node& stat, Graph& graph, const bfs::path& filename1,
	...
	try {
		...
		for (unsigned int i = 0; i < 3; ++i) {
			if (stat.getParam("quality").getDoubleValueAtIndex(i)
...
EImageStatus diffFile(Graph::Node& read1, Graph::Node& read2, Graph::Node& stat,
		Graph& graph, const bfs::path& filename1, const bfs::path& filename2) {
	EImageStatus s = diffImageStatus(read1, read2, stat, graph, filename1,
			filename2);
...
void diffSequence(Graph::Node& read1, Graph::Node& read2, Graph::Node& stat,
		Graph& graph, const Sequence& seq1, const Sequence& seq2) {
...
void diffSequence(Graph::Node& read1, Graph::Node& read2, Graph::Node& stat,
		Graph& graph, const Sequence& seq1, const Sequence& seq2,
		const Sequence::Time first, const Sequence::Time last) {
...
int main(int argc, char** argv) {
	...
	try {
		...
		hidden.add_options()(kEnableColorOptionLongName.c_str(),
				bpo::value<std::string>(), kEnableColorOptionMessage.c_str());
		...
		try {
			...
			bpo::store(
					bpo::command_line_parser(argc, argv).options(
							cmdline_options).positional(pod).run(), vm);

			// get environment options and parse them
			if (const char* env_diff_options = std::getenv(
					"SAM_DIFF_OPTIONS")) {
				const std::vector<std::string> vecOptions = bpo::split_unix(
						env_diff_options, " ");
				bpo::store(
						bpo::command_line_parser(vecOptions).options(
								cmdline_options).positional(pod).run(), vm);
			}
		...
		if (vm.count(kHelpOptionLongName.c_str())) {
			//std::cout.rdbuf(_stdCout); // restore cout's original streambuf
			TUTTLE_COUT(
					_color._blue << "TuttleOFX project [http://sites.google.com/site/tuttleofx]" << _color._std << std::endl);
			TUTTLE_COUT( _color._blue << "NAME" << _color._std);
			TUTTLE_COUT(
					_color._green << "\tsam-diff - compute difference between 2 images/sequences" << _color._std << std::endl);
			TUTTLE_COUT( _color._blue << "SYNOPSIS" << _color._std);
			TUTTLE_COUT(
					_color._green << "\tsam-diff [reader] [input] [reader] [input] [options]" << _color._std << std::endl);
			TUTTLE_COUT(
					_color._blue << "DESCRIPTION" << _color._std << std::endl);

			TUTTLE_COUT( "diff if sequence have black images.");
			TUTTLE_COUT(
					"This tools process the PSNR of an image, and if it's null, the image is considered black.");
		...
		if (!vm.count(kReaderOptionLongName.c_str())) {
			TUTTLE_COUT(
					_color._red << "sam-diff : no reader specified." << _color._std);
			TUTTLE_COUT(
					_color._red << "           run sam-diff -h for more information." << _color._std);
			return 0;
		}
		if (!vm.count(kInputDirOptionLongName.c_str())) {
			TUTTLE_COUT(
					_color._red << "sam-diff : no input specified." << _color._std);
			TUTTLE_COUT(
					_color._red << "           run sam-diff -h for more information." << _color._std);
			return 0;
		}
		readerId = vm[kReaderOptionLongName.c_str()].as<std::vector<std::string> >();
		inputs = vm[kInputDirOptionLongName.c_str()].as<std::vector<std::string> >();
		if (readerId.size() != 2 || inputs.size() != 2) {
			TUTTLE_COUT(
					_color._red << "sam-diff : number of input or reader is/are incorrect." << _color._std);
			TUTTLE_COUT(
					_color._red << "           run sam-diff -h for more information." << _color._std);
			return 0;
		}
		...
		if (bfs::exists(path1)) {
			...
			try {
				...
				Sequence s2(path2);
				if (hasRange) {
					diffSequence(read1, read2, stat, graph, s1, s2, range[0],
							range[1]);
				} else {
					diffSequence(read1, read2, stat, graph, s1, s2);
				}
			} catch (...) {
				std::cerr << "Unrecognized pattern " << path1 << " or " << path2
						<< std::endl;
