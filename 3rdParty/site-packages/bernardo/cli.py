...
class Brndo(cmd.Cmd):
    ...
    def do_configure(self, arg):
        ...
        for lib in libs:
            ...
            if not lib.get('configure_cmd'):
                ...
                continue
            if tool.execute_command(lib, 'configure_cmd', self.extracted_dir, verbose_output):
                print "Ok - Configured."
            else:
    ...
    def do_build(self, arg):
        ...
        Build lib.
          -v  Show command output.
          -f  Full build (fetch, shacheck, extract and build)
        '''
        libs = self.all_libs
        verbose = False
        full_build = False
        ...
        #TODO Add all libs dependencies in libs if not already present
        #TODO Ordered libs by topology
        for lib in libs:
            if full_build:
                #TODO if lib.installed is True:
                #TODO     continue
                print "Full build."
                if verbose:
                    self.onecmd(" ".join(['fetch -v', lib['name']]))
                    self.onecmd(" ".join(['shacheck -v', lib['name']]))
                    self.onecmd(" ".join(['extract -v -f', lib['name']]))
                    self.onecmd(" ".join(['configure -v', lib['name']]))
                else:
                    self.onecmd(" ".join(['fetch', lib['name']]))
                    self.onecmd(" ".join(['shacheck', lib['name']]))
                    self.onecmd(" ".join(['extract -f', lib['name']]))
                    self.onecmd(" ".join(['configure', lib['name']]))
                # for step in full_build_order[0:full_build_order.index('build')]:
                #     func = getattr(self, 'do_' + step)
                #     if not verbose:
                #         func(lib['name'])
                #     else:
                #         func(" ".join(['-v', lib['name']]))

            print "-- \nBuild ", lib['name']
            if not lib.get('build_cmd'):
                print "*Warning* - Build command not specified. (Skipping)"
                continue
            if tool.execute_command(lib, 'build_cmd', self.extracted_dir, verbose):
                print "Ok - Built."
            else:
    ...
    def do_install(self, arg):
        ...
        for lib in libs:
            if verbose:
                ...
                self.onecmd(" ".join(['extract -f', lib['name']]))
                self.onecmd(" ".join(['configure', lib['name']]))
            # for step in full_build_order[0:full_build_order.index('install')]:
            #     func = getattr(self, 'do_' + step)
            #     if not verbose:
            #         func(lib['name'])
            #     else:
