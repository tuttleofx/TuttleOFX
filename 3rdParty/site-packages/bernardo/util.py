...
def check_file_size(filename, size):
    '''
    Return 0 if os.stat(filename) has posix.stat_result.st_size equal size
...
def fetch_archive(filename, total_size):
    '''
    '''
    import os
    real_path = os.path.join(os.environ['BRNDO_ROOT'], 'archive', filename)
    if os.path.exists(real_path):
         current_size = check_file_size(real_path, total_size)
