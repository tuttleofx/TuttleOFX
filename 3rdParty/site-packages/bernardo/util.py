...
###new
def archive_is_uncompressed(filename):
    import os, tarfile

    if not os.path.exists(filename):
        return False

    tar = tarfile.open(os.path.join(os.environ['BRNDO_ROOT'], 'archive', 
                                    os.path.basename(filename)))
    
    
def check_sha1(content, sha1, verbose=True):
    '''
...
def fetch_archive(url, ressource_name, expected_size, verbose=True):
    '''
    ...
    urllib.urlretrieve(url, ressource_name, report_hook)
...
def is_fetched(filename, total_size):
    '''
    ...
    if check_file_size(filename, total_size):
        return False
...
def untar(filename, dest_dir, verbose=True):
    '''
