...
void exportAsDOT<TestVertex, TestEdge>(std::ostream& os, const InternalGraph<TestVertex, TestEdge>& g)
{
    ...
    boost::write_graphviz(os, g.getGraph(), make<vertex_label_writer>(g.getGraph()), make<edge_label_writer>(g.getGraph()),
...
BOOST_AUTO_TEST_CASE(test_export)
{
    ...
    //	boost::depth_first_search( graph.getGraph(), boost::root_vertex(nodesDescriptor[n1]), boost::visitor(testVisitorA)
    //);//, colormap );
...
BOOST_AUTO_TEST_CASE(check_cycle)
{
    ...
    BOOST_CHECK_THROW(graph.addEdge(nodesDescriptor[n3], nodesDescriptor[n2], graph::TestEdge(n3, n2)), exception::Logic);
    BOOST_CHECK_THROW(graph.addEdge(nodesDescriptor[n3], nodesDescriptor[n1], graph::TestEdge(n3, n1)), exception::Logic);
...
{
    using namespace tuttle::host;
    typedef boost::adjacency_list<boost::listS, boost::vecS, boost::undirectedS, graph::TestVertex, graph::TestEdge>
        TestGraph;
    TestGraph graph;
    ...
    {
        TestGraph::edge_descriptor ed = boost::add_edge(graph.vertex_set()[i], graph.vertex_set()[i + 1], graph).first;
    }
