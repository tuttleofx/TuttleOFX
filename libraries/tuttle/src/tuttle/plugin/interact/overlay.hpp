...
{
    glBegin(GL_LINE_STRIP);
    for(typename std::vector<Point2>::const_iterator it = vec.begin(), itEnd = vec.end(); it != itEnd; ++it)
    {
...
{
    for(typename std::vector<std::vector<Point2> >::const_iterator it = vec.begin(), itEnd = vec.end(); it != itEnd; ++it)
    {
        drawCurve(*it);
    }
}
