...
{
public:
    ParamRectangleFromTwoCorners(const InteractInfos& infos, OFX::Double2DParam* paramA, OFX::Double2DParam* paramB,
...
public:
    ...
    Point2 getPoint() const
    {
        ...
        return pointNormalizedXXcToCanonicalXY(ofxToGil((_paramB->getValue() + _paramA->getValue()) * 0.5), rodSize) +
...
template <class TFrame, ECoordinateSystem coord>
ParamRectangleFromTwoCorners<TFrame, coord>::ParamRectangleFromTwoCorners(const InteractInfos& infos,
...
ParamRectangleFromTwoCorners<TFrame, coord>::selectType(const OFX::PenArgs& args) const
{
...
bool ParamRectangleFromTwoCorners<TFrame, coord>::isIn(const OfxRectD& rect)
{
    ...
    max.x = std::max(a.x, b.x);
    max.y = std::max(a.y, b.y);
    if(rect.x1 <= min.x && rect.x2 >= max.x && rect.y1 <= min.y && rect.y2 >= max.y)
    {
        return true;
    }
    return false;
