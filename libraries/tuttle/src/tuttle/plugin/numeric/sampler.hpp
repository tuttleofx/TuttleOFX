...
struct bicubic1D
{
	...
	void operator( )( const SrcP& srcA, const SrcP& srcB, const SrcP& srcC, const SrcP& srcD, F weight, DstP & dst ) const
	{
		...
		ttl_detail::add_dst_mul_src<SrcP, float, DstP > ( )( srcA, -weight * ( 1 + valueB ), mp );
		ttl_detail::add_dst_mul_src<SrcP, float, DstP > ( )( srcB, 1 + ( weight * valueB ), mp );
		ttl_detail::add_dst_mul_src<SrcP, float, DstP > ( )( srcC, weight * ( 1 - valueA ), mp );
		ttl_detail::add_dst_mul_src<SrcP, float, DstP > ( )( srcD, weight*valueA, mp );
...
void setXPixels( const xy_locator& loc, const point2<std::ptrdiff_t>& p0, const diff p, int windowWidth, SrcP& ptA, SrcP& ptB, SrcP& ptC, SrcP& ptD )
{
	...
	ptA = p0.x > 0 && p0.x - 1 < windowWidth ? loc.x( )[-1] : *loc;
	ptB = p0.x < windowWidth ? *loc : *loc;
	ptC = p0.x + 1 < windowWidth ? loc.x( )[1] : *loc;
	ptD = p0.x + 2 < windowWidth ? loc.x( )[2] : *loc;
}
