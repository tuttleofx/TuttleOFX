...
void appendPluginFactory(OFX::PluginFactoryArray& ids, const char* idString, const int versionMajor, const int versionMinor,
                         const char* uniqueId)
{
    static const char* uniqueIdStr =
        uniqueId; // Make the uniqueId present in the final binary, even in release. OFXPLUGIN_tuttle.myplug_1.0
...
/**
 * @brief Use this macro to declare your plugin in the mainEntry.cpp if you have plugins who don't have the same version in
 * the bundle.
 */
#define mAppendPluginFactoryVersions(ids, ClassName, idString, versionMajor, versionMinor)                                  \
    tuttle::plugin::appendPluginFactory<ClassName>(                                                                         \
        ...
        "OFXPLUGIN_" idString "_" BOOST_PP_STRINGIZE(versionMajor) "." BOOST_PP_STRINGIZE(versionMinor));
