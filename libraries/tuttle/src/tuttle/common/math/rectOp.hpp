...
inline OfxRectD pointsBoundingBox(const std::vector<Point2>& points)
{
    ...
    for(typename std::vector<Point2>::const_iterator it = points.begin(), itEnd = points.end(); it != itEnd; ++it)
    {
        if(it->x < bounds.x1)
            bounds.x1 = it->x;
...
inline Point2 pointsMinXY(const std::vector<Point2>& points)
{
    ...
    for(typename std::vector<Point2>::const_iterator it = points.begin() + 1, itEnd = points.end(); it != itEnd; ++it)
    {
        if(it->x < p.x)
            p.x = it->x;
        if(it->y < p.y)
