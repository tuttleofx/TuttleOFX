...
void Graph::deleteNode(Node& node)
{
    ...
    if(it == _nodesMap.end())
    {
        BOOST_THROW_EXCEPTION(exception::Value() << exception::user("Node not found."));
    }
    removeFromInternalGraph(node);
...
void Graph::connect(const std::list<std::string>& nodes)
{
    ...
    if(nodes.size() <= 1)
        BOOST_THROW_EXCEPTION(exception::Logic() << exception::user("Needs multiple nodes to connect them together."));
...
void Graph::connect(const std::vector<Node*>& nodes)
{
    ...
    if(nodes.size() <= 1)
        BOOST_THROW_EXCEPTION(exception::Logic() << exception::user("Needs multiple clips to connect them together."));
...
void Graph::replaceNodeConnections(const Node& fromNode, const Node& toNode)
{
    ...
    BOOST_FOREACH(edge_descriptor e, _graph.getInEdges(_graph.getVertexDescriptor(fromNode.getName())))
    {
        ...
        connect(_graph.sourceInstance(e).getProcessNode(), toNode.getAttribute(_graph.instance(e).getInAttrName()));
    }
    BOOST_FOREACH(edge_descriptor e, _graph.getOutEdges(_graph.getVertexDescriptor(fromNode.getName())))
    {
        ...
        connect(toNode, _graph.targetInstance(e).getProcessNode().getAttribute(_graph.instance(e).getInAttrName()));
    }
    unconnect(fromNode);
}
...
std::vector<const Graph::Node*> Graph::getNodes() const
{
    ...
    for(NodeMap::const_iterator it = getNodesMap().cbegin(), itEnd = getNodesMap().cend(); it != itEnd; ++it)
    {
...
std::vector<Graph::Node*> Graph::getNodes()
{
    ...
    nodes.reserve(getNodesMap().size());
    for(NodeMap::iterator it = getNodesMap().begin(), itEnd = getNodesMap().end(); it != itEnd; ++it)
    {
        nodes.push_back(it->second);
    }
    return nodes;
