...
void OfxhImageEffectPluginCache::loadFromPlugin(OfxhPlugin& op)
{
    ...
    if(rval != kOfxStatOK && rval != kOfxStatReplyDefault)
    {
        ...
        BOOST_THROW_EXCEPTION(exception::OfxCustom(rval) << exception::user("Loading plugin failed.")
    ...
    if(rval != kOfxStatOK && rval != kOfxStatReplyDefault)
    {
        BOOST_THROW_EXCEPTION(exception::OfxCustom(rval) << exception::user("Unloading plugin failed in initialization.")
...
OfxhPlugin* OfxhImageEffectPluginCache::newPlugin(OfxhPluginBinary& pb, int pi, OfxPlugin& pl)
{
...
OfxhPlugin* OfxhImageEffectPluginCache::newPlugin(OfxhPluginBinary& pb, int pi, const std::string& api, int apiVersion,
                                                  ...
                                                  int pluginMajorVersion, int pluginMinorVersion)
{
    OfxhImageEffectPlugin* plugin =
        new OfxhImageEffectPlugin(*this, pb, pi, api, apiVersion, pluginId, rawId, pluginMajorVersion, pluginMinorVersion);
...
std::ostream& operator<<(std::ostream& os, const OfxhImageEffectPluginCache& v)
{
    ...
    for(OfxhImageEffectPluginCache::MapPluginsByID::const_iterator it = v._pluginsByID.begin(); it != v._pluginsByID.end();
        ++it)
    {
