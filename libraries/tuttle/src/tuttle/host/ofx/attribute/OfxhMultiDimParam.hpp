...
public:
    ...
    void setValueFromExpression(const std::string& value, const ofx::attribute::EChange change) OFX_EXCEPTION_SPEC
    {
        ...
        else
        {
            ...
            {
                BOOST_THROW_EXCEPTION(exception::Value() << exception::user() + "Set " + allExp.size() +
...
protected:
    ...
    inline virtual void getValueAtIndex(const std::size_t index, BaseType& outDst) const OFX_EXCEPTION_SPEC
    {
        if(index >= getSize())
        {
            BOOST_THROW_EXCEPTION(exception::BadIndex() << exception::user() + "Get value at index " + index +
                                                               " on a multidimensional parameter of size " + DIM + ".");
        }
        _controls[index].getValue(outDst);
    }

    inline virtual void getValueAtTimeAndIndex(const OfxTime time, const std::size_t index,
...
public:
    ...
    inline void setValueAtTime(const OfxTime time, const BaseType& value,
    ...
    inline virtual void setValueAtIndex(const std::size_t index, const BaseType& value,
    ...
    inline virtual void setValue(const std::vector<BaseType>& values, const EChange change) OFX_EXCEPTION_SPEC
    {
        ...
        {
            BOOST_THROW_EXCEPTION(exception::BadIndex() << exception::user() + "Set " + values.size() +
                                                               " values on a multidimensional parameter of size " + DIM +
                                                               ".");
        }
        for(std::size_t i = 0; i < getSize(); ++i)
    ...
    inline virtual void setValueAtTime(const OfxTime time, const std::vector<BaseType>& values,
                                       const EChange change) OFX_EXCEPTION_SPEC
    {
        if(values.size() != getSize())
        {
    ...
    inline virtual void deriveAtIndex(const OfxTime time, const std::size_t index, BaseType& outDst) const OFX_EXCEPTION_SPEC
    {
        if(index >= getSize())
        {
            BOOST_THROW_EXCEPTION(exception::BadIndex() << exception::user() + "Derive value at index " + index +
                                                               " on a multidimensional parameter of size " + DIM + ".");
        }
        _controls[index].derive(time, outDst);
    }

    inline virtual void integrateAtIndex(const OfxTime time1, const OfxTime time2, const std::size_t index,
                                         BaseType& outDst) const OFX_EXCEPTION_SPEC
    {
        if(index >= getSize())
        {
            BOOST_THROW_EXCEPTION(exception::BadIndex() << exception::user() + "Integrate value at index " + index +
                                                               " on a multidimensional parameter of size " + DIM + ".");
        }
