...
public:
    ...
    void finish_vertex(VertexDescriptor vd, Graph& g)
    {
        ...
        if(vertex.getProcessNode().isIdentity(vertex.getProcessDataAtTime(), inputClip, atTime))
        {
                    ...
                    if(!clipFound)
                    {
                        ...
                        BOOST_THROW_EXCEPTION(
                            ...
                            << exception::dev() + "Plugin declares itself as identity, but gives a non existing input clip. "
                                                  "(node: " +
                                   vertex.getName() + ", input clip:" + inputClip + ", existing clips: (" +
                                   existingClips.str() + "))");
                    }
                    if(!clipFoundAtTime)
                    {
                        ...
                        BOOST_THROW_EXCEPTION(
                            exception::Bug() << exception::dev() + "Plugin declares itself as identity, but gives a time on "
                                                                   "the input clip different from declared framesNeeded. "
                                                                   "(node: " +
                                                    vertex.getName() + ", input clip:" + inputClip + ", time:" + atTime +
                                                    ", framesNeeded:(" + framesNeeded.str() + "))");
                    }
            ...
            {
                e << exception::user() + "Error on the node " + quotes(vertex.getName()) +
                         ". It fails to declare itself as an identity node."
                  << exception::nodeName(vertex.getName()) << exception::time(vertex.getProcessDataAtTime()._time);
                throw;
            }
        }
    }
...
void removeIdentityNodes(TGraph& graph, const std::vector<IdentityNodeConnection<TGraph> >& nodesToRemove)
{
    ...
    BOOST_FOREACH(const IdentityNodeConnection<TGraph>& connection, nodesToRemove)
    {
        ...
        else
        {
            ...
            TUTTLE_LOG_TRACE("IN: " << boost::lexical_cast<std::string>(connection._identityVertex)
                                    << "::" << boost::lexical_cast<std::string>(connection._input._inputClip) << " <<-- "
...
public:
    ...
    void finish_vertex(VertexDescriptor v, const Graph& g)
    {
        ...
        if(!vertex.isFake())
        {
            ...
            vertex.getProcessNode().preProcess_infos(vertex.getProcessDataAtTime(), procOptions._time,
...
public:
    ...
    void finish_vertex(VertexDescriptor v, Graph& g)
    {
        ...
        TUTTLE_LOG_TRACE("[Process] " << quotes(vertex._name) << " " << vertex._data._time << " took: " << t2 - t1
                                      << " (cumul: " << _cumulativeTime << ")" << vertex);
        ...
        if(_result && vertex.getProcessDataAtTime()._isFinalNode)
        {
            ...
            if(!img.get())
            {
                BOOST_THROW_EXCEPTION(exception::Logic()
                                      << exception::user() +
                                             "Output buffer not found in memoryCache at the end of the node process."
                                      << exception::dev() + vertex._clipName + "." kOfxOutputAttributeName + " at time " +
                                             vertex._data._time
                                      << exception::nodeName(vertex._name) << exception::time(vertex._data._time));
            }
