...
void InternalGraph<VERTEX, EDGE, OutEdgeList, VertexList, EdgeList>::toDominatorTree()
{
    ...
    domTreePredVector =
        std::vector<vertex_descriptor>(num_vertices(_graph), boost::graph_traits<GraphContainer>::null_vertex());
...
template <typename VERTEX, typename EDGE, typename OutEdgeList, typename VertexList, typename EdgeList>
std::size_t
InternalGraph<VERTEX, EDGE, OutEdgeList, VertexList, EdgeList>::removeUnconnectedVertices(const vertex_descriptor& vroot)
{
...
{
    os << "  vertex count: " << g.getVertexCount() << std::endl << "  edge count: " << g.getEdgeCount() << std::endl;
