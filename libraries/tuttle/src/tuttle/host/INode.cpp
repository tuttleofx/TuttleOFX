...
INode::Data& INode::getData()
{
    if(!_data)
    {
        BOOST_THROW_EXCEPTION(exception::Bug() << exception::dev("Process data not set.") << exception::nodeName(getName()));
    }
    return *_data;
}
...
const INode::DataAtTime& INode::getData(const OfxTime time) const
{
    ...
    if(it == _dataAtTime.end())
    {
        ...
        BOOST_THROW_EXCEPTION(exception::Bug() << exception::dev() + "Process data at time not set.\n" + ss.str()
...
const INode::DataAtTime& INode::getFirstData() const
{
    ...
    if(it == _dataAtTime.end())
    {
        BOOST_THROW_EXCEPTION(exception::Bug() << exception::dev() + "Process data empty."
                                               << exception::nodeName(getName()));
    }
    return *it->second;
}
