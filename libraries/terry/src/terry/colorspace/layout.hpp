...
struct pixel_color_layout_t
{
    ...
    {
        static_for_each(p1, p2, channel_color_layout_t<Channel, IN, OUT>(_in, _out));
        return p2;
    }
};
...
void layout_convert_view(const View& src, View& dst, const LayoutIN& layoutIn = LayoutIN(),
...
void layout_convert_pixel(const Pixel& src, Pixel& dst, const LayoutIN& layoutIn = LayoutIN(),
                          const LayoutOUT& layoutOut = LayoutOUT())
{
