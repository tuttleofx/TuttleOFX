 ...
 */
template <typename Scalar>
terry::filter::kernel_1d<Scalar> buildGaussian1DKernel(const Scalar size, const bool normalize = true,
...
Scalar gaussianDerivativeValueAt(const Scalar x, const Scalar amplitude, const Scalar yscale = 1, const Scalar xcenter = 0)
{
    ...
    return (/*-*/ x / yscale) * std::exp(-bm::pow<2>(x - xcenter) / (2.0 * amplitude)) /
 ...
 */
template <typename Scalar>
terry::filter::kernel_1d<Scalar> buildGaussianDerivative1DKernel(const Scalar size, const bool normalize = true,
                                                                 const double epsilon = kConvolutionEpsilon)
{
    using namespace boost;
    if(size == 0)
