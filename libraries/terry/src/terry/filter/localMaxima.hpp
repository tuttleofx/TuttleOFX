...
struct pixel_locator_gradientLocalMaxima_t
{
    ...
    DPixel operator()(const SLocator& src) const
    {
        ...
        // A
        if(((*src)[vecY] <= 0 && (*src)[vecX] > -(*src)[vecY]) || ((*src)[vecY] >= 0 && (*src)[vecX] < -(*src)[vecY]))
        {
            SType d = 0.0;
            if((*src)[vecX])
            {
        ...
        // B
        else if(((*src)[vecX] > 0 && -(*src)[vecY] >= (*src)[vecX]) || ((*src)[vecX] < 0 && -(*src)[vecY] <= (*src)[vecX]))
        {
            SType d = 0.0;
            if((*src)[vecY])
            {
        ...
        // C
        else if(((*src)[vecX] <= 0 && (*src)[vecX] > (*src)[vecY]) || ((*src)[vecX] >= 0 && (*src)[vecX] < (*src)[vecY]))
        {
            SType d = 0.0;
            if((*src)[vecY])
            {
...
void applyLocalMaxima(const SView& srcView, DView& dstView)
{
    ...
    terry::algorithm::transform_pixels_locator(srcView, getBounds<std::ptrdiff_t>(srcView), dstView,
