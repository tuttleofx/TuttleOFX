...
void sobel(const SView& srcView, const DView& dstViewX, const DView& dstViewY, const point2<double>& size,
           const convolve_boundary_option boundary_option)
{
    ...
    kernel_1d<Scalar> xKernelGaussianDerivative =
        buildGaussianDerivative1DKernel<Scalar>(size.x, normalizedKernel, kernelEpsilon);
    ...
    kernel_1d<Scalar> yKernelGaussianDerivative =
        buildGaussianDerivative1DKernel<Scalar>(size.y, normalizedKernel, kernelEpsilon);
    kernel_1d<Scalar> yKernelGaussian = buildGaussian1DKernel<Scalar>(size.y, normalizedKernel, kernelEpsilon);

    correlate_rows_cols_auto<DPixelGray, Alloc>(color_converted_view<DPixelGray>(srcView), xKernelGaussianDerivative,
                                                xKernelGaussian, dstViewX, proc_tl, boundary_option);

    correlate_rows_cols_auto<DPixelGray, Alloc>(color_converted_view<DPixelGray>(srcView), yKernelGaussian,
