...
template <typename SrcConstRefP, typename DstP,
          typename CC> // const_reference to the source pixel and destination pixel value
class color_process_deref_fn
    : public deref_base<color_process_deref_fn<SrcConstRefP, DstP, CC>, DstP, DstP, const DstP&, SrcConstRefP, DstP, false>
{
private:
...
public:
    ...
    color_process_deref_fn(const CC& cc_in)
        : _cc(cc_in)
    {
    }
...
public:
    ...
    static type make(const View& sv, const CC& cc) { return add_ref_t::make(sv, deref_t(cc)); }
};
