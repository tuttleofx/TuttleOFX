...
inline boost::gil::point2<F> transform(const PinningPerspective<F>& t, const boost::gil::point2<F2>& src)
{
    ...
    res.x = pOut[0] / pOut[2];
    res.y = pOut[1] / pOut[2];
    // res.x = (t._matrix(0, 0) * pIn[0] + t._matrix(0, 1) * pIn[1] + t._matrix(0, 2)) / (t._matrix(2, 0) * pIn[0] +
    // t._matrix(2, 1) * pIn[1] + t._matrix(2, 2));
    // res.y = (t._matrix(1, 0) * pIn[0] + t._matrix(1, 1) * pIn[1] + t._matrix(1, 2)) / (t._matrix(2, 0) * pIn[0] +
    // t._matrix(2, 1) * pIn[1] + t._matrix(2, 2));
