...
OFX::Clip *
DPXReaderPlugin::getDstClip( ) const
{
    return _dstClip;
}
 ...
 */
void
DPXReaderPlugin::render( const OFX::RenderArguments &args )
{
...
void
DPXReaderPlugin::changedParam( const OFX::InstanceChangedArgs &args, const std::string &paramName )
{
    if( paramName == "Help" )
    {
...
void
DPXReaderPlugin::getRegionsOfInterest(const OFX::RegionsOfInterestArguments &args, OFX::RegionOfInterestSetter &rois) {
    const OfxRectD roi = { 0.0, 0.0, _imageDims.x * _dstClip->getPixelAspectRatio(), _imageDims.y };
    rois.setRegionOfInterest(*_dstClip, roi);
}

bool
DPXReaderPlugin::getRegionOfDefinition(const OFX::RegionOfDefinitionArguments &args, OfxRectD &rod) {
    rod.x1 = 0;
