...
void ColorCubeProcess<View>::setup( const OFX::RenderArguments& args )
{
    ...
    ColorCubeVirtualView colorCube( ColorCubePoint( size, size ), ColorCubeLocator( ColorCubePoint( 0, 0 ), ColorCubePoint( 1, 1 ), ColorCubeFunctorT( tileSize, _params.step ) ) );
    // create a subview depending on the image ratio
    _srcColorCubeView = subimage_view<>( colorCube, xshift, yshift, boost::numeric_cast<int>( dims.x ), boost::numeric_cast<int>( dims.y ) );
}
