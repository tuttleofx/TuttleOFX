...
void ColorWheelProcess<View>::setup( const OFX::RenderArguments& args )
{
	...
	ColorWheelVirtualView checker( Point( size, size ), Locator( Point( 0, 0 ), Point( 1, 1 ), ColorWheelFunctorT( tileSize ) ) );
...
void ColorWheelProcess<View>::multiThreadProcessImages( const OfxRectI& procWindowRoW )
{
	...
	for( int y = procWindowOutput.y1;
			 ...
			 ++y )
	{
		typename ColorWheelVirtualView::x_iterator src_it = this->_srcView.x_at( procWindowOutput.x1, y );
