...
void ColorWheelProcess<View>::setup( const OFX::RenderArguments& args )
{
	...
	else
	{
		...
		// create a squared color wheel
		ColorWheelVirtualView colorWheel( ColorWheelPoint( size, size ), ColorWheelLocator( ColorWheelPoint( 0, 0 ), ColorWheelPoint( 1, 1 ), ColorWheelFunctorT( tileSize, _params.mode == eColorWheelModeBlack ) ) );
		// create a subview depending on the image ratio
		_srcColorWheelView = subimage_view<>( colorWheel, xshift, yshift, boost::numeric_cast<int>( dims.x ), boost::numeric_cast<int>( dims.y ) );
	}
}
