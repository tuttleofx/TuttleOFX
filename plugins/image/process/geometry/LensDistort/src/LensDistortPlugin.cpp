    ...
    if(paramName == kParamLensType)
    {
        switch((EParamLensType)_lensType->getValue())
        {
            ...
            case eParamLensTypeFisheye4:
                ...
                break;
            default:
                BOOST_THROW_EXCEPTION(exception::Bug() << exception::user() + "Lens type value not recognize.");
                break;
        }
    }
...
bool LensDistortPlugin::isIdentity(const OFX::RenderArguments& args, OFX::Clip*& identityClip, double& identityTime)
{
    ...
    else if(_coef1->getValue() == 0 && _preScale->getValue() == _preScale->getDefault() &&
            _postScale->getValue() == _postScale->getDefault() && _preOffset->getValue() == _preOffset->getDefault() &&
            _postOffset->getValue() == _postOffset->getDefault() && (!_coef2->getIsEnable() || _coef2->getValue() == 0) &&
...
bool LensDistortPlugin::getRegionOfDefinition(const OFX::RegionOfDefinitionArguments& args, OfxRectD& rod)
{
    ...
    switch(static_cast<EParamResizeRod>(_resizeRod->getValue()))
    {
        ...
        case eParamResizeRodMin:
        {
            ...
            Point2 tr =
                transformValues(getLensType(), params, Point2(srcRodInDstFrame.x2, srcRodInDstFrame.y1)); // top right
            Point2 bl =
                transformValues(getLensType(), params, Point2(srcRodInDstFrame.x1, srcRodInDstFrame.y2)); // bottom left
            Point2 br =
                transformValues(getLensType(), params, Point2(srcRodInDstFrame.x2, srcRodInDstFrame.y2)); // bottom right
...
LensDistortPlugin::getProcessParams(const OfxRectD& inputRod, const OfxRectD& outputRod, const OfxRectD& optionalInputRod,
                                    const double pixelAspectRatio, const bool reverse) const
{
    const bool useOptionalInputRod =
        (optionalInputRod.x1 != optionalInputRod.x2) && (optionalInputRod.y1 != optionalInputRod.y2);
    ...
    switch((EParamNormalization)_normalization->getValue())
    {
        ...
        case eParamNormalizationDiagonal:
            params.normalizeCoef =
                std::sqrt(params.imgSizeSrc.x * params.imgSizeSrc.x + params.imgSizeSrc.y * params.imgSizeSrc.y);
            break;
        case eParamNormalizationHalfDiagonal:
            params.normalizeCoef =
                std::sqrt(params.imgCenterSrc.x * params.imgCenterSrc.x + params.imgCenterSrc.y * params.imgCenterSrc.y);
            break;
            ...
            if(useOptionalInputRod)
            {
                Point2 imgShiftBetweenInputs =
                    Point2(optionalInputRod.x1 - inputRod.x1,
                           optionalInputRod.y1 - inputRod.y1); // translate inputRod -> optionalInputRod
