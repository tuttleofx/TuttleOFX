...
//	changedParam( _changedArgs, kParamMode );
      ...
      OfxRectI
      CropPlugin::getCropRegionValue() const
      {
        OfxRectI cropRegion;
      ...
      int
      CropPlugin::safeModulo(const int &cropValue, const int &rodValue)
      {
      ...
      OfxRectI
      CropPlugin::computeCropRegion(const OfxTime time,
...
//		}
        ...
        else if (paramName == kParamPreset)
          {
            double ratio = 0.0;
            EParamPreset preset =
                static_cast<EParamPreset>(_paramPreset->getValue());
      ...
      bool
      CropPlugin::getRegionOfDefinition(
          const OFX::RegionOfDefinitionArguments& args, OfxRectD& rod)
      {

        CropProcessParams<rgba32f_pixel_t> params = getProcessParams<
            rgba32f_pixel_t>(args.time);
