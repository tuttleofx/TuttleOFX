 ...
 */
void OverlayData::computeHistogramBufferData(HistogramBufferData& data, SView& srcView, const OfxTime time,
                                  ...
                                  const bool selectionOnly)
{
    ...
    boost::scoped_ptr<OFX::Image> src(
        clipSrc->fetchImage(time, clipSrc->getCanonicalRod(time, renderScale))); // scoped pointer of current source clip
    // TUTTLE_LOG_INFOS;
...
void OverlayData::computeCurveFromSelectionData(OFX::Clip* clipSrc, const OfxTime time, const OfxPointD& renderScale)
{
    ...
    boost::scoped_ptr<OFX::Image> src(clipSrc->fetchImage(
        _currentTime, clipSrc->getCanonicalRod(_currentTime, renderScale))); // scoped pointer of current source clip
