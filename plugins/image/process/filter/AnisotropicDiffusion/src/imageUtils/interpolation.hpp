 ...
 */
inline const float& 
pix2d(const float *W, const int x, const int y, const int c, 
      const int w, const int h, const int nc) {
    return W[((y < 0 ? 0 : (y >= h ? h - 1 : y)) * w + 
              (x < 0 ? 0 : (x >= w ? w - 1 : x))) * nc + c];
}
...
const float
pix2d(DstView &myView, const int x, const int y, 
 ...
 */
inline float 
linear_pix2d(const float *W, const float fx, const float fy, const int c, 
             const int w, const int h, const int nc) {
    const int
...
template <typename DstView>
float 
linear_pix2d(DstView &myView, const float fx, const float fy, 
             const int c, const int w, const int h) {
    const int
        x = (int)fx - (fx >= 0 ? 0 : 1), nx = x + 1,
        y = (int)fy - (fy >= 0 ? 0 : 1), ny = y + 1;
