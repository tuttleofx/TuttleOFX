...
{
	TUTTLE_COUT( "******** PROCESS READER JPEG2000 ********" );
	Graph g;

	TUTTLE_COUT( "--> PLUGINS CREATION" );
	Graph::Node& read = g.createNode( "tuttle.jpeg2000reader" );

	TUTTLE_COUT( "--> PLUGINS CONFIGURATION" );

	read.getParam( "filename" ).setValue( "data/file.j2k" );
...
{
	TUTTLE_COUT( "******** PROCESS READER JPEG2000 NO FILE ********" );
	Graph g;
