#include <iostream>
#define BOOST_TEST_MODULE plugin_Jpeg2000_writer
...
BOOST_AUTO_TEST_CASE( process_reader )
{
	...
	g.compute( outputCache, writer, options );
...
{
	TUTTLE_COUT( "******** PROCESS WRITER JPEG2000 NO FILE ********" );
	Graph g;

	TUTTLE_COUT( "--> PLUGINS CREATION" );
	Graph::Node& read = g.createNode( "tuttle.jpeg2000writer" );

	TUTTLE_COUT( "--> PLUGINS CONFIGURATION" );
	read.getParam( "filename" ).setValue( "data/no-such-file.j2k" );
