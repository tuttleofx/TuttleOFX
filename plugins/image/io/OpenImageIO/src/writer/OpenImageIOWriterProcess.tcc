...
void OpenImageIOWriterProcess<View>::writeImage( View& src, const std::string& filepath, const ETuttlePluginBitDepth& bitDepth )
{
	...
	if( out.get() == NULL )
	{
		BOOST_THROW_EXCEPTION( exception::Bug()
							   << exception::user( "OIIO Writer: error from writer while opening file." ) );
	}
	...
	switch( finalBitDepth )
	{
		case eTuttlePluginBitDepthAuto:
			BOOST_THROW_EXCEPTION( exception::Bug()
								   << exception::user( "OIIO Writer: undefined output bit depth." ) );
		case eTuttlePluginBitDepth8:
