...
OutputBufferProcessParams OutputBufferPlugin::getProcessParams() const
{
    ...
    params._callbackDestroyPtr =
        reinterpret_cast<CallbackDestroyCustomDataPtr>(stringToPointer(_paramCallbackDestroyCustomData->getValue()));
    return params;
}
...
void OutputBufferPlugin::render(const OFX::RenderArguments& args)
{
    ...
    if(params._callbackPtr != NULL)
    {
        params._callbackPtr(args.time, params._customDataPtr, rawImagePtrLink, bounds.x2 - bounds.x1, bounds.y2 - bounds.y1,
