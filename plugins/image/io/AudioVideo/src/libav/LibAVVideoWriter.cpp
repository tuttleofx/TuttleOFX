...
void LibAVVideoWriter::optionSet( const EAVParamType& type, const AVOption& opt, bool& value )
{
	int error = 0;
	switch( type )
	{
	...
	if( error )
		TUTTLE_CERR( "avwriter: " << libavError_toString( error ) << " : " << opt.name << " ( " << ( value ? "True" : "False" ) << " )" );
}

void LibAVVideoWriter::optionSet( const EAVParamType& type, const AVOption &opt, bool& value, std::string& valueToSetFlag )
{
	int error = 0;
	int64_t optVal;
	void* obj = NULL;
	...
	if( error )
		TUTTLE_CERR( "avwriter: " << libavError_toString( error ) << " : " << valueToSetFlag << " ( " <<  opt.name << " = " << ( value ? "True" : "False" ) << " )" );
}

void LibAVVideoWriter::optionSet( const EAVParamType& type, const AVOption& opt, int& value )
{
	int error = 0;
	switch( type )
	{
...
void LibAVVideoWriter::optionSet( const EAVParamType& type, const AVOption &opt, int &valueNum, int& valueDen )
{
	int error = 0;
	AVRational q;
	q.num = valueNum;
	...
	if( error )
		TUTTLE_CERR( "avwriter: " << libavError_toString( error ) << " : " << opt.name << " ( " << valueNum << "/" << valueDen<< " )" );
}

void LibAVVideoWriter::optionSet( const EAVParamType& type, const AVOption &opt, std::string &value )
{
	int error = 0;
	if( ! value.length() )
		return;
...
void LibAVVideoWriter::optionSetImageSize( const EAVParamType &type, const AVOption &opt, int &width, int& height)
{
	/*
	switch( type )
	{
