 ...
 */
void AVWriterPluginFactory::describeInContext(OFX::ImageEffectDescriptor& desc, OFX::EContext context)
{
    ...
    {
        formatPresetParam->appendOption((*it).find(avtranscoder::constants::avProfileIdentificator)->second + ": " +
    ...
    for(avtranscoder::NamesMap::const_iterator itName = formatsNames.begin(); itName != formatsNames.end(); ++itName)
    {
    ...
    {
        videoPresetParam->appendOption((*it).find(avtranscoder::constants::avProfileIdentificator)->second + ": " +
                                       (*it).find(avtranscoder::constants::avProfileIdentificatorHuman)->second);
    }
    ...
    {
        audioMainPresetParam->appendOption((*it).find(avtranscoder::constants::avProfileIdentificator)->second + ": " +
                                           (*it).find(avtranscoder::constants::avProfileIdentificatorHuman)->second);
    }
    ...
    for(avtranscoder::NamesMap::const_iterator itName = audioCodecsNames.begin(); itName != audioCodecsNames.end(); ++itName)
    {
    ...
    audioNbInputs->setHint("An 'Input' allow you to create one of several audio streams in your output file. It could be by "
                           "rewrap or transcode one or several audio streams of an input file, or by generate a dummy audio "
                           "stream.");
    ...
    for(size_t indexAudioInput = 0; indexAudioInput < maxNbAudioInput; ++indexAudioInput)
    {
        ...
        for(avtranscoder::ProfileLoader::Profiles::iterator it = audioPresets.begin(); it != audioPresets.end(); ++it)
        {
            audioPresetParam->appendOption((*it).find(avtranscoder::constants::avProfileIdentificator)->second + ": " +
                                           (*it).find(avtranscoder::constants::avProfileIdentificatorHuman)->second);
        }
    ...
    hints[kParamMetaAlbumArtist] =
        "main creator of the set/album, if different from artist. (e.g. 'Various Artists' for compilation albums.)";
    ...
    hints[kParamMetaLanguage] = "main language in which the work is performed, preferably in ISO 639-2 format. Multiple "
                                "languages can be specified by separating them with commas.";
    hints[kParamMetaPerformer] = "artist who performed the work, if different from artist. ( E.g for 'Also sprach "
                                 "Zarathustra', artist would be 'Richard Strauss' and performer 'London Philharmonic "
                                 "Orchestra'.";
    ...
    for(ritKeys = keys.begin(), ritHint = hints.begin(); ritKeys != keys.end(); ++ritKeys, ++ritHint)
    {
 ...
 */
OFX::ImageEffect* AVWriterPluginFactory::createInstance(OfxImageEffectHandle handle, OFX::EContext context)
{
