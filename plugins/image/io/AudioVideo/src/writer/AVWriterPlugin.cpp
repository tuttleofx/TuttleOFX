    ...
    , _initWrap(false)
{
    ...
    common::disableOFXParamsForFormatOrCodec(*this, _libavFeatures._optionsPerOutputFormat, formatName,
                                             common::kPrefixFormat);
    common::disableOFXParamsForFormatOrCodec(*this, _libavFeatures._optionsPerVideoCodec, videoCodecName,
                                             common::kPrefixVideo);
    common::disableOFXParamsForFormatOrCodec(*this, _libavFeatures._optionsPerAudioCodec, audioCodecName,
                                             common::kPrefixAudio);
...
{
    if(_paramAudioSubGroup.at(indexAudioOutput)->getIsEnable() && !_paramAudioSubGroup.at(indexAudioOutput)->getIsSecret())
    {
...
void AVWriterPlugin::changedParam(const OFX::InstanceChangedArgs& args, const std::string& paramName)
{
    ...
    else if(paramName == kParamAudioMainPreset)
    {
        ...
        if(_paramAudioMainPreset->getValue() == 0 || _paramAudioMainPreset->getValue() == 1)
        {
...
void AVWriterPlugin::initVideo(const OFX::RenderArguments& args)
{
    ...
    {
        BOOST_THROW_EXCEPTION(exception::Failed() << exception::user() + "no output file indicated"
    ...
    if(!isOutputInit())
    {
        BOOST_THROW_EXCEPTION(exception::Failed() << exception::user() + "the output is not init. Can't process.");
    }
    ...
    try
    {
        ...
        const avtranscoder::ProfileLoader::Profile videoDetailProfile =
            _paramVideoDetailCustom.getCorrespondingProfile(params._videoCodecName);
    ...
    {
        _initVideo = false;
        BOOST_THROW_EXCEPTION(exception::Failed() << exception::user() + "unable to create video stream: " + e.what());
    }
    _initVideo = true;
}
...
void AVWriterPlugin::initAudio()
{
    ...
    try
    {
        ...
        for(int i = 0; i < _paramAudioNbInput->getValue(); ++i)
        {
            ...
            if(presetIndex == 0 && mainPresetIndex == 0)
            {
                ...
                avtranscoder::ProfileLoader::Profile audioDetailProfile =
                    _paramAudioDetailCustom.getCorrespondingProfile(params._audioCodecName);
    ...
    {
        BOOST_THROW_EXCEPTION(exception::Failed() << exception::user() + "unable to create audio stream: " + e.what());
    }
}

bool AVWriterPlugin::isOutputInit()
{
    if(_outputFile.get() == NULL || _transcoder.get() == NULL)
        return false;
    return true;
}
    ...
    if(presetIndex != 0)
    {
        ...
        BOOST_FOREACH(const avtranscoder::ProfileLoader::Profile::value_type& option, existingProfile)
        {
            ...
            if(!_paramFormatCustom.setOption(option.first, option.second))
                _paramFormatDetailCustom.setOption(option.first, option.second,
                                                   existingProfile.at(avtranscoder::constants::avProfileFormat));
        }
    }
}
...
void AVWriterPlugin::updateVideoFromExistingProfile()
{
    ...
    if(presetIndex != 0)
    {
        ...
        {
            const std::vector<std::string> pixelFormats(
                avtranscoder::getSupportedPixelFormats(existingProfile.at(avtranscoder::constants::avProfileCodec)));
            std::vector<std::string>::const_iterator iterPixelFormat = std::find(
                pixelFormats.begin(), pixelFormats.end(), existingProfile.at(avtranscoder::constants::avProfilePixelFormat));
        ...
        if(existingProfile.find(avtranscoder::constants::avProfileFrameRate) != existingProfile.end())
            _paramCustomFps->setValue(
                boost::lexical_cast<double>(existingProfile.at(avtranscoder::constants::avProfileFrameRate)));
        else
        ...
        BOOST_FOREACH(const avtranscoder::ProfileLoader::Profile::value_type& option, existingProfile)
        {
            ...
            if(!_paramVideoCustom.setOption(option.first, option.second))
                _paramVideoDetailCustom.setOption(option.first, option.second,
                                                  existingProfile.at(avtranscoder::constants::avProfileCodec));
        }
    }
}
...
void AVWriterPlugin::updateAudioFromExistingProfile()
{
    ...
    if(presetIndex > 1)
    {
        ...
        {
            const std::vector<std::string> sampleFormats(
                avtranscoder::getSupportedSampleFormats(existingProfile.at(avtranscoder::constants::avProfileCodec)));
            std::vector<std::string>::const_iterator iterSampleFormat =
                std::find(sampleFormats.begin(), sampleFormats.end(),
                          existingProfile.at(avtranscoder::constants::avProfileSampleFormat));
        ...
        BOOST_FOREACH(const avtranscoder::ProfileLoader::Profile::value_type& option, existingProfile)
        {
            ...
            if(!_paramAudioCustom.setOption(option.first, option.second))
                _paramAudioDetailCustom.setOption(option.first, option.second,
                                                  existingProfile.at(avtranscoder::constants::avProfileCodec));
        }
    }
}
...
void AVWriterPlugin::render(const OFX::RenderArguments& args)
{
    ...
    if(!_initWrap)
    {
        ...
        const avtranscoder::ProfileLoader::Profile formatDetailProfile =
            _paramFormatDetailCustom.getCorrespondingProfile(params._formatName);
...
std::string AVWriterPlugin::getPixelFormatName(const std::string& videoCodecName) const
{
    ...
    catch(const std::exception& e)
    {
        ...
        BOOST_THROW_EXCEPTION(exception::Failed() << exception::user() +
                                                         "unable to get supported pixel format choosen for video codec " +
                                                         videoCodecName + " - Supported pixel format are: " +
                                                         supportedPixelFormatList + "- " + e.what());
    }
}
...
std::string AVWriterPlugin::getSampleFormatName(const std::string& audioCodecName) const
{
    ...
    catch(const std::exception& e)
    {
        ...
        BOOST_THROW_EXCEPTION(exception::Failed() << exception::user() +
                                                         "unable to get supported sample format choosen for audio codec " +
                                                         audioCodecName + " - Supported sample format are: " +
                                                         supportedSampleFormatList + "- " + e.what());
    }
}
...
void AVWriterPlugin::setFormatParam(const std::string& formatShortName)
{
    ...
    for(common::LibAVFeaturesAvailable::NamesArray::const_iterator itFormat = formats.begin(); itFormat != formats.end();
        ++itFormat)
    {
...
void AVWriterPlugin::setVideoCodecParam(const std::string& videoCodecShortName)
{
    ...
    for(common::LibAVFeaturesAvailable::NamesArray::const_iterator itCodec = codecs.begin(); itCodec != codecs.end();
        ++itCodec)
    {
