	...
	, _allSupportedSampleFormats(avtranscoder::getSupportedSampleFormats())
{
    ...
    for( avtranscoder::NamesMap::const_iterator itFormat = formatsAsMap.begin();
    itFormat != formatsAsMap.end();
    ++itFormat )
    {
    ...
    for( avtranscoder::NamesMap::const_iterator itCodec = videoCodecsAsMap.begin();
    itCodec != videoCodecsAsMap.end();
    ++itCodec )
    {
    ...
    for( avtranscoder::NamesMap::const_iterator itCodec = audioCodecsAsMap.begin();
    itCodec != audioCodecsAsMap.end();
    ++itCodec )
    {
