...
ColorCubeProcessParams<ColorCubePlugin::Scalar> ColorCubePlugin::getProcessParams( const OfxPointD& renderScale ) const
{
	ColorCubeProcessParams<Scalar> params;                   // create parameters container object
 ...
 */
void ColorCubePlugin::changedParam( const OFX::InstanceChangedArgs &args, const std::string &paramName )
{
 ...
 * If clip has changed
 */
void ColorCubePlugin::changedClip( const OFX::InstanceChangedArgs& args, const std::string& clipName )
{
...
bool ColorCubePlugin::isIdentity( const OFX::RenderArguments& args, OFX::Clip*& identityClip, double& identityTime )
{
//	ColorCubeProcessParams<Scalar> params = getProcessParams();
...
void ColorCubePlugin::render( const OFX::RenderArguments &args )
{
	...
	doGilRender<ColorCubeProcess>( *this, args ); //launch process
}
 ...
 * @param args
 */
void ColorCubePlugin::updateGeodesicForms(const OFX::InstanceChangedArgs& args)
{
 ...
 * @param args
 */
void ColorCubePlugin::updateProcessGeodesicForms(const OFX::RenderArguments &args)
{
	//update time
	...
	if(_paramChoiceAverageMode->getValue() ==0) //average mode is automatic
	{
		...
		_renderAttributes.geodesicFormColor.subdiviseFaces(selectionAverage._averageValue, _paramIntDiscretization->getValue()); //create geodesic form
		_renderAttributes.geodesicFormSpill.subdiviseFaces(selectionAverage._averageValue, _paramIntDiscretization->getValue()); //create geodesic form
	}
	else //average mode is manual
	{
		...
		_renderAttributes.geodesicFormColor.subdiviseFaces(selectedAverage, _paramIntDiscretization->getValue()); //create geodesic form
		_renderAttributes.geodesicFormSpill.subdiviseFaces(selectedAverage, _paramIntDiscretization->getValue()); //create geodesic form
	}
	//Extend geodesic forms
	selectionAverage.extendGeodesicForm(_clipColor,_renderScale,_renderAttributes.geodesicFormColor);	//extends geodesic form color
	_renderAttributes.geodesicFormSpill.copyGeodesicForm(_renderAttributes.geodesicFormColor);							//extends geodesic form spill (color clip)
	selectionAverage.extendGeodesicForm(_clipSpill,_renderScale,_renderAttributes.geodesicFormSpill);	//extends geodesic form spill (spill takes account of spill clip)
...
void ColorCubePlugin::addRefCloudPointData()
{
...
void ColorCubePlugin::releaseCloudPointData()
{
...
bool ColorCubePlugin::hasCloudPointData() const
{
	return _cloudPointDataCount != 0;
}

CloudPointData& ColorCubePlugin::getCloudPointData()
{
	return *_cloudPointData.get();
}
const CloudPointData& ColorCubePlugin::getCloudPointData() const
{
	return *_cloudPointData.get();
}
/// @}
