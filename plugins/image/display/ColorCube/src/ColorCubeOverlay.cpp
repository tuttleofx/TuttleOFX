 ...
 * @param effect
 */
ColorCubeOverlay::ColorCubeOverlay(OfxInteractHandle handle,OFX::ImageEffect* effect)
: OFX::OverlayInteract(handle)
, _infos(effect)
{	
	_plugin = static_cast<ColorCubePlugin*>(_effect); //get plugin
 ...
 * Destructor
 */
ColorCubeOverlay::~ColorCubeOverlay() 
{
 ...
 */
void ColorCubeOverlay::prepareOpenGLScene(const OFX::DrawArgs& args)
{	
 ...
 */
void ColorCubeOverlay::drawAxes()
{
	glBegin(GL_LINES);
	//X axis (black => red)
 ...
 */
bool ColorCubeOverlay::penDown( const OFX::PenArgs& args )
{
 ...
 * @return if the event has been treated (y or n)
 */
bool ColorCubeOverlay::penUp( const OFX::PenArgs& args )
{
 ...
 * @return if the event has been treated (y or n)
 */
bool ColorCubeOverlay::penMotion( const OFX::PenArgs& args )
{
 ...
 * @return  if the event has been treated (y or n)
 */
bool ColorCubeOverlay::keyDown( const OFX::KeyArgs& args )
{
 ...
 * @return  if the event has been treated (y or n)
 */
bool ColorCubeOverlay::keyUp( const OFX::KeyArgs& args )
{
	if(args.keySymbol==kOfxKey_Control_L||args.keySymbol==kOfxKey_Control_R) //if the pressed key is Ctrl key (left or right)
	{
		if(_isCtrlKeyDown)
...
/*
 * Get cloud point data from ColorCubePlugin
 * @return 
 */
CloudPointData& ColorCubeOverlay::getData()
{
 ...
 */
void ColorCubeOverlay::updateModelView(const Ofx3DPointD& rotationCenter)
{	
	//Define and create new transformation matrix
	Matrix4 newTransformationMatrix;																	//initialize new transformation matrix
	if(rotationCenter.x == 0.5 && rotationCenter.y == 0.5 && rotationCenter.z == 0.5)					//if current rotation center is reference center
		newTransformationMatrix = constructRotationMatrix(rotationCenter,_rotateY,_rotateX,0);			//construct new transformation matrix
	else																								//current rotation center is geodesic form center
		newTransformationMatrix = constructRotationMatrix(rotationCenter,_rotateYForm,_rotateXForm,0);	//construct new transformation matrix
 ...
 */
void ColorCubeOverlay::drawWarning(const Ofx3DPointD& centerPoint, const double ratio)
{
