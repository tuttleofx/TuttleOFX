#ifndef _TUTTLE_PLUGIN_DIFFPLOT_PLUGIN_HPP_
#define _TUTTLE_PLUGIN_DIFFPLOT_PLUGIN_HPP_
...
/**
 * @brief DiffPlot plugin
 */
class DiffPlotPlugin : public ImageEffectGilPlugin
{
public:	
    typedef float Scalar;
	bool _isCleaned;								//ask overlay to Reset scene
	bool _isNbStepChanged;							//do we have to recompute selection diffPlots (nbStep has changed)
	bool _isDiffPlotRefresh;						//do we have to recompute selection diffPlots (other reason)
	
	// Plugin clips
    OFX::Clip *_clipSrcB; ///< Destination image clip
    ...
    OFX::ChoiceParam* _paramDisplayTypeSelection;	//display option list global/adapted to channel (DiffPlot overlay group)
...
public:
    DiffPlotPlugin( OfxImageEffectHandle handle );
	DiffPlotProcessParams<Scalar> getProcessParams( const OfxTime time, const OfxPointD& renderScale = OFX::kNoRenderScale ) const;
