...
struct merge_functor_impl<Pix, mpl::true_> {
	...
	// User have to implement the following
	inline void operator()(const value_t & A, const value_t & B,
							  const value_t & a, const value_t & b,
							  value_t & d);
	inline void merge(const Pix & A, const Pix & B, Pix & d);
};
...
: public detail::merge_functor_impl<Pix, mpl::true_>
{
	...
	inline void operator()(const value_t & A, const value_t & B, value_t & d) {
