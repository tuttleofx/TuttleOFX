...
template <template <typename> class Functor,
          typename SrcChannelAV, typename SrcChannelBV, typename DstChannelV>
// Model ChannelValueConcept
struct channel_merging : public std::binary_function<SrcChannelAV, SrcChannelBV, DstChannelV>
{
	DstChannelV operator()( SrcChannelAV srcA, SrcChannelBV srcB ) const
	{
		return Functor<DstChannelV>::merge( srcA, srcB );
	}

	template <typename ItA, typename ItB>
	DstChannelV operator()( const ItA& itA, const ItA& itB, SrcChannelAV srcA, SrcChannelBV srcB ) const
	{
...
inline typename channel_traits<DstChannel>::value_type channel_merge( const ItA& itA, const ItB& itB, SrcChannelA& srcA, SrcChannelB& srcB )
{
...
template <template <typename> class Functor,
          typename DstChannel, typename SrcChannelA, typename SrcChannelB>
inline typename channel_traits<DstChannel>::value_type channel_merge( SrcChannelA& srcA, SrcChannelB& srcB )
{
	return channel_merging<
	           Functor,
	           typename channel_traits<SrcChannelA>::value_type,
